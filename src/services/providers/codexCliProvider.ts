// Codex CLI provider — routes through local Vite dev server middleware
// No API key required; uses the authenticated `codex` CLI on the host machine.
// Only functional in dev mode (the /api/codex-cli endpoint is not available in production).

import { CONTEXT_PROMPT } from './prompts';
import { generateMockFeedback } from './mockFeedback';

const CLI_ENDPOINT = '/api/codex-cli';

function getCliModel(): string {
  return localStorage.getItem('codex_cli_model')
    || (import.meta.env.VITE_CODEX_CLI_MODEL as string | undefined)
    || '';
}
